...
def cmd_doctor(_: argparse.Namespace) -> int:
    ...
    for module in ["numpy", "scipy", "cv2", "yaml", "skimage", "sklearn", "casadi", "quadprog", "trajectory_planning_helpers"]:
        try:
            m = importlib.import_module(module); print(f"{module}: OK {getattr(m, '__version__', '(no __version__)')}")
        except Exception as exc:
            failures.append((module, str(exc))); print(f"{module}: FAIL {exc}")
    try:
        ...
        verify_tph_spline_approximation_compat(); print(f"Vendor: OK {checkout}\nTPH/SciPy spline compatibility: OK")
    except Exception as exc:
        failures.append(("vendor/compat", str(exc))); print(f"Vendor/compat: FAIL {exc}")
    if failures:
...
def cmd_edit(args: argparse.Namespace) -> int:
    ...
    run_editor(map_yaml=args.map, profile_path=args.profile, sector_profile_path=args.sector_profile, raceline_dir=args.raceline_dir, host=args.host, port=args.port, open_browser=not args.no_browser)
    return 0
...
def _write_outputs(out: Path, result) -> None:
    ...
    save_csv(out / "bound_right.csv", result.bound_right, "x_m,y_m"); save_csv(out / "bound_left.csv", result.bound_left, "x_m,y_m")
