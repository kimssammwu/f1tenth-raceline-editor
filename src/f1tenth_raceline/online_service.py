    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path.startswith("/download/"):
                key = path.rsplit("/", 1)[-1]; root = projects.get(key)
                if not root: self.send_json({"error":"project not found"}, 404); return
                self.send_data(_zip_project(root), "application/zip", disposition=f'attachment; filename="raceline-{key}.zip"'); return
            self.send_json({"error":"not found"}, 404)
        def do_POST(self):
            if urlparse(self.path).path != "/api/upload": self.send_json({"error":"not found"}, 404); return
            try:
                ...
                probe = ThreadingHTTPServer((host, 0), BaseHTTPRequestHandler); editor_port = probe.server_port; probe.server_close()
                threading.Thread(target=run_editor, kwargs={"map_yaml":map_yaml,"host":host,"port":editor_port,"open_browser":False}, daemon=True).start()
                self.send_json({"ok":True,"map":yamls[0],"editor":f"http://{host}:{editor_port}","download":f"/download/{quote(key)}"})
